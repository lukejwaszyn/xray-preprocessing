...
%
% Run with no arguments to launch file-picker dialogs:
%     preprocess_xray()
%
...
%     'object'  -- path to object TIFF (required)
%     'dark'    -- path to dark TIFF (optional)
%     'flat'    -- path to flat TIFF (optional)
%     'out'     -- output folder (default: 'outputs')
%     'prefix'  -- filename prefix to keep multiple runs separate
%
% Author:  Luke Waszyn
% Lab:     Penn State HEATER Lab -- Experimental Frost Setup
%
% All processing preserves grayscale intensity. The output 16-bit TIFFs
% retain ~65,000 levels of detail for downstream ML.

% ----------------------------------------------------------------------
% Pipeline parameters -- tweak here if needed
% ----------------------------------------------------------------------
PCT_LOW    = 1;       % clip the bottom 1% of pixels (kills dead pixels)
PCT_HIGH   = 99;      % clip the top 1% of pixels (kills hot pixels)
CLAHE_CLIP = 0.01;    % CLAHE strength: higher = more contrast, more noise
GAMMA      = 0.6;     % gamma < 1 brightens midtones, > 1 darkens
...
% Interactive file pickers if no object path was supplied
if isempty(opt.object)
    fprintf('============================================================\n');
    fprintf('  HEATER Lab -- X-ray Preprocessing\n');
    fprintf('============================================================\n\n');

    [f,d] = uigetfile({'*.tif;*.tiff','TIFF';'*.*','All'}, ...
                      'Select OBJECT TIFF (required)');
    ...
    pickedOut = uigetdir(pwd, 'Select OUTPUT folder');
...
% ----------------------------------------------------------------------
% Drop empty/partial pages (some software saves blank frames when a
% capture fails). Real frames are 95%+ nonzero; junk frames are <50%.
% ----------------------------------------------------------------------
keep = [];
for k = 1:size(obj, 3)
    nonzero_pct = 100 * nnz(obj(:,:,k)) / numel(obj(:,:,k));
    if nonzero_pct > 50
...
frames = obj(:,:,keep);
fprintf('[frames] using %d of %d frames: %s\n', ...
        numel(keep), size(obj,3), mat2str(keep));

% Average dark/flat across pages if they're stacks (we want a single 2-D ref)
if ~isempty(dark) && size(dark,3) > 1
    dark = mean(double(dark), 3);
end
if ~isempty(flat) && size(flat,3) > 1
    flat = mean(double(flat), 3);
end

% ----------------------------------------------------------------------
% Mean-stacked frame: averaging N frames reduces shot noise by sqrt(N).
...
for i = 1:numel(allRaw)
    ...
    % Stage 2: percentile normalization to [0, 1]
    normalized = percentileNormalize(corrected, PCT_LOW, PCT_HIGH);

    % Stage 3: contrast enhancement (two flavors)
    %   adapthisteq is MATLAB's built-in CLAHE
    clahe = adapthisteq(normalized, 'ClipLimit', CLAHE_CLIP, ...
                                    'NumTiles', [8 8]);
    %   imadjust with a 4th argument applies a gamma curve
    gammaImg = imadjust(normalized, [], [], GAMMA);

    % Save: 16-bit TIFF (full precision, for ML) + 8-bit PNG (for viewing)
    ...
    % 4-panel comparison figure for the lab
...
% Load any TIFF (single or multi-page) as an H x W x N array.
% Uses the Tiff class for cross-version reliability with multi-page files.
    info = imfinfo(path);
    n = numel(info);

    t = Tiff(path, 'r');
    cleanupObj = onCleanup(@() close(t));   % auto-close even on error
...
% Falls back gracefully if dark or flat are missing.
    out = double(raw);
    if ~isempty(dark)
        out = out - double(dark);
    end
    if ~isempty(flat)
        ...
        flatVal(flatVal <= 0) = 1;   % guard against dead pixels
        out = out ./ flatVal;
    end
end
...
% Stretch the image so [pLow, pHigh] percentiles map to [0, 1].
% Pixels keep their relative ordering -- a darker pixel stays darker,
% just rescaled to a 0-1 float. Only the extremes get clamped.
    lo = prctile(img(:), pLow);
...
function saveComparisonFigure(raw, normalized, clahe, gammaImg, outpath, titleStr)
    ...
    subplot(2,2,2); imshow(normalized,[0 1]); title('Percentile-normalized'); colorbar;
...
% Tiny ternary helper -- MATLAB doesn't have a one-liner for this.
